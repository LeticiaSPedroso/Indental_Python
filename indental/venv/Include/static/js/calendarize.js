			...
			daysInMonth.forEach(function(c, d) {
				...
				if (opts.clickHandler && !$dayNode.classList.contains('dummy-day')) {
					function handleEvent(e) {
						e = e || window.event;
						// chama o método para formatar a data
						var newDate = FormatDate(c);
						// inclui a data no input
						document.getElementById("txtData").value = newDate;
						// redireciona para a página desta cadeira nessa data
						e.preventDefault();
						e.stopPropagation();
						var touches = false;
						if (!touches) {
...
// Função para formatar a data vinda do calendario
